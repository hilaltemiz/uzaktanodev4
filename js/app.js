$(document).ready(function(){
    ...
    document.querySelectorAll('.card').forEach(function(card) {
      card.addEventListener('click', function() {
        if(didGameStart === false) {
          // ilk tıklamayla zamanlayıcıyı ayarla
          didGameStart = true;
        ...
        if (card === lastFlipped || matches.includes(card) || pause || isGameOver) {
          // kartların kendileriyle karşılaştırılmasını veya oyun bittiğinde oynamayı engeller
          return;
        }

        card.classList.add('open', 'show');

        if (lastFlipped) { // önceki bir kart tıklandı; son tıklamayı bu tıklamayla karşılaştır
  ...
  /*
    henüz eşleşmemiş bir kart bulur,
     kart sınıfını al, sonra eşleşen diğerini bul,
     sonra her ikisini de birkaç dakika gösterir
  */
  function hint() {
  ...
  function resetGame(e) {
    ...
    // temizler ve kartları yeniden oluşturur
    clearDeck();
    generateCards();
    activateCards();
    flash_cards();
  ...
  /*show / open sınıflarını ekleyin, zaman aşımından sonra kaldır */
